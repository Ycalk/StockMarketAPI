...
async def test_user_lifecycle_with_balances(ctx: dict):
    # 1. Create user
    create_response: CreateUserResponse = await Users.create_user(
        ctx, CreateUserRequest(name="Full Lifecycle User")
    )
    ...
    balance_response: GetBalanceResponse = await Users.get_balance(
        ctx, get_balance_request
    )
    ...
    balance_withdrawal: GetBalanceResponse = await Users.get_balance(
        ctx, get_balance_request
    )
    ...
    with pytest.raises(InsufficientFundsError):
        bad_withdraw_request = WithdrawRequest(
            user_id=user_id, ticker="USD", amount=300
        )
        await Users.withdraw(ctx, bad_withdraw_request)

    # 8. Verify balance didn't change after failed withdrawal
    balance_failed_withdrawal: GetBalanceResponse = await Users.get_balance(
        ctx, get_balance_request
    )
...
async def test_multiple_instruments_operations(ctx: dict):
    # 1. Create user
    user: UserSharedModel = (
        await Users.create_user(ctx, CreateUserRequest(name="Multi Instrument User"))
    ).user
    ...
    await Users.deposit(
        ctx, DepositRequest(user_id=user.id, ticker=usd.ticker, amount=1000)
    )
    await Users.deposit(
        ctx, DepositRequest(user_id=user.id, ticker=eur.ticker, amount=500)
    )
    await Users.deposit(
        ctx, DepositRequest(user_id=user.id, ticker=gbp.ticker, amount=300)
    )

    # 4. Verify all balances
    balance_response: GetBalanceResponse = await Users.get_balance(
        ctx, GetBalanceRequest(user_id=user.id)
    )
    ...
    await Users.withdraw(
        ctx, WithdrawRequest(user_id=user.id, ticker=usd.ticker, amount=200)
    )
    await Users.deposit(
        ctx, DepositRequest(user_id=user.id, ticker=eur.ticker, amount=200)
    )

    # 6. Verify new balances
    new_balance_response: GetBalanceResponse = await Users.get_balance(
        ctx, GetBalanceRequest(user_id=user.id)
    )
...
async def test_error_handling_scenarios(ctx: dict):
    ...
    with pytest.raises(UserNotFoundError):
        await Users.deposit(
            ctx, DepositRequest(user_id=uuid.uuid4(), ticker="USD", amount=100)
        )

    # 2. Create real user but try invalid operations
    user: UserSharedModel = (
        await Users.create_user(ctx, CreateUserRequest(name="Error Handling User"))
    ).user

    with pytest.raises(InstrumentNotFoundError):
        await Users.deposit(
            ctx, DepositRequest(user_id=user.id, ticker="XYZ", amount=100)
        )

    with pytest.raises(InstrumentNotFoundError):
        await Users.withdraw(
            ctx, WithdrawRequest(user_id=user.id, ticker="USD", amount=100)
        )
...
async def test_user_deletion_with_balances(ctx: dict):
    ...
    user: UserSharedModel = (
        await Users.create_user(ctx, CreateUserRequest(name="Delete User"))
    ).user
    usd = await Instrument.create(ticker="USD", name="US Dollar")
    await Users.deposit(
        ctx, DepositRequest(user_id=user.id, ticker=usd.ticker, amount=1000)
...
async def test_concurrent_operations(ctx: dict):
    ...
    user: UserSharedModel = (
        await Users.create_user(ctx, CreateUserRequest(name="Concurrent User"))
    ).user
    instrument = await Instrument.create(ticker="USD", name="US Dollar")
    ...
    async def deposit_task(amount):
        await Users.deposit(
            ctx,
            DepositRequest(user_id=user.id, ticker=instrument.ticker, amount=amount),
        )
...
async def test_high_volume_transactions(ctx: dict):
    # 1. Create user and instrument
    user: UserSharedModel = (
        await Users.create_user(ctx, CreateUserRequest(name="High Volume User"))
    ).user
    ...
    for i in range(1, 101):
        await Users.deposit(
            ctx, DepositRequest(user_id=user.id, ticker=usd.ticker, amount=i)
        )

        if i % 10 == 0:
            await Users.withdraw(
                ctx, WithdrawRequest(user_id=user.id, ticker=usd.ticker, amount=5)
            )

    # 3. Check final balance
    expected_balance = sum(range(1, 101)) - (5 * 10)
    balance: GetBalanceResponse = await Users.get_balance(
        ctx, GetBalanceRequest(user_id=user.id)
    )
    ...
    deposit_count = await BalanceHistory.filter(
        user_id=user.id, operation_type=OperationType.DEPOSIT
    ).count()

    withdraw_count = await BalanceHistory.filter(
        user_id=user.id, operation_type=OperationType.WITHDRAW
    ).count()
...
async def test_balance_consistency_after_errors(ctx: dict):
    ...
    user: UserSharedModel = (
        await Users.create_user(ctx, CreateUserRequest(name="Consistency User"))
    ).user
    usd = await Instrument.create(ticker="USD", name="Dollar")
    await Users.deposit(
        ctx, DepositRequest(user_id=user.id, ticker=usd.ticker, amount=100)
    )

    initial_balance: GetBalanceResponse = await Users.get_balance(
        ctx, GetBalanceRequest(user_id=user.id)
    )
    ...
    current_balance: GetBalanceResponse = await Users.get_balance(
        ctx, GetBalanceRequest(user_id=user.id)
    )
    ...
    with pytest.raises(InstrumentNotFoundError):
        await Users.deposit(
            ctx, DepositRequest(user_id=user.id, ticker="XYZ", amount=50)
        )
...
async def test_user_operations_with_multiple_instruments(ctx: dict):
    ...
    for ticker, amount, op_type in operations:
        if op_type == "deposit":
            await Users.deposit(
                ctx, DepositRequest(user_id=user.id, ticker=ticker, amount=amount)
            )
        else:
            await Users.withdraw(
                ctx, WithdrawRequest(user_id=user.id, ticker=ticker, amount=amount)
            )

    # 4. Check final balances
    final_balances: GetBalanceResponse = await Users.get_balance(
        ctx, GetBalanceRequest(user_id=user.id)
    )
...
async def test_concurrent_balance_updates(ctx: dict):
    ...
    assert (
        await BalanceHistory.filter(user=user_model, instrument=instrument).count()
        == 50
    ), "Balance history count mismatch"
