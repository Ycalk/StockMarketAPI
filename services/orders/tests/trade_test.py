...
async def test_limit_buy_order_trading_partial_fill(
    ...
    await Balance.create(user=seller, instrument=instrument, amount=seller_instrument_amount)
    ...
    assert seller_rub_balance.amount == seller_rub_amount + transaction_amount * transaction_price
    assert buyer_rub_balance.amount == buyer_rub_amount - transaction_amount * transaction_price

    assert seller_instrument_balance.amount == seller_instrument_amount - transaction_amount
    assert buyer_instrument_balance.amount == buyer_instrument_amount + transaction_amount
...
async def test_limit_buy_order_trading_with_yourself_partial_fill(
    ...
    await Balance.create(user=user, instrument=instrument, amount=instrument_init_balance)
    ...
    instrument_balance, _ = await Balance.get_or_create(user=user, instrument=instrument)
